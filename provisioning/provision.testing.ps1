...
function CreateUser($userinfo) {
	...
	if ( test-user -samAccountName $userinfo.samaccountname -adDomainControllerHostName $DC ) {
		...
		# add LARC/GBLS attorneys to the "attorneys" distribution list and paralegals to the "paralegals" distribution list
		if ( ($userinfo.title -match "Attorney" ) ) {
			if (($userinfo.department -match "LARC")) {
				$null = Add-ADGroupMember -identity "_Attorneys-LARC" -members $userinfo.samAccountName
			} else {
				$null = Add-ADGroupMember -identity "_Attorneys-GBLS" -members $userinfo.samAccountName
			}
		}
		
		if ( ($userinfo.title -match "Paralegal" ) ) { 
			$null = Add-ADGroupMember -identity "_Paralegals-GBLS" -members $userinfo.samAccountName
		}
...
# based on user's department, casls/cambridge, and intern/full time staff
function getUserSharedFolderPath($ht) {

		if ($ht.department -eq "CASLS") {
			$userSharedFolder = $cambridgeShare
		} else {
			$userSharedFolder = $bostonShare
		}
...
function getUserSharedFolderPathOther($ht) {
		...
		if ($ht.department -eq "CASLS") {
			$userSharedFolderOther = $cambridgeDrive
		} else {
			$userSharedFolderOther = $bostonDrive
		}
...
$csvFile = get-FileName("\\gbls\Shares\SharedBoston")
